...
@section('content')
            ...
            <div>
                <div class="d-flex align-items-center gap-2 mb-2">
                    ...
                    <span class="badge bg-primary text-white rounded-pill px-3 py-2 fw-bold shadow-sm" style="backdrop-filter: blur(5px);">
                        Total: {{ $treasurers->total() }} Data
                    </span>
                </div>
                <h4 class="fw-bold text-white mb-1"><i class="ti tabler-wallet me-2"></i>Manajemen Bendahara Penerimaan</h4>
                <p class="text-white-50 mb-0" style="font-size: 0.85rem;">Kelola data bendahara penerimaan, riwayat jabatan, dan akses sistem.</p>
            </div>
        </div>
        <i class="ti tabler-wallet position-absolute text-white" style="font-size: 160px; right: -15px; bottom: -30px; opacity: 0.08; transform: rotate(-10deg); z-index: 1;"></i>
    </div>
    ...
    <div class="glass-card overflow-hidden anim-2">
            ...
            <div class="card-title mb-0">
                ...
                <p class="text-muted mb-0">Menampilkan {{ $treasurers->count() }} dari total {{ $treasurers->total() }} bendahara.</p>
            </div>
            <div class="d-flex justify-content-md-end align-items-center gap-3">
                ...
                <button type="button" class="btn btn-primary rounded-pill shadow-sm btn-action" id="btn-add-treasurer" data-bs-toggle="modal" data-bs-target="#treasurerModal">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-hover">
                    <thead class="table-light">
                        <tr>
                            <th class="ps-4 text-primary fw-bold text-uppercase" style="font-size: 0.75rem;">Bendahara</th>
                            ...
                            <tr class="premium-row {{ !$isActive ? 'bg-lighter' : '' }}">
                                ...
                                <td class="text-center pe-4">
                                    <div class="d-flex align-items-center justify-content-center gap-1">
                                        <a class="btn btn-sm btn-icon btn-label-info rounded-circle btn-action" href="{{ route('admin.treasurers.show', $treasurer->id) }}" data-bs-toggle="tooltip" title="Lihat Profil">
                                            <i class="ti tabler-eye icon-20px"></i>
                                        </a>
                                        @if(Auth::user()->role !== 'leader')
                                        {{-- Tombol Edit --}}
                                        <button type="button" class="btn btn-sm btn-icon btn-label-primary rounded-circle btn-action btn-edit-treasurer"
                                        ...
                                        @if($isActive)
                                            <button type="button" class="btn btn-sm btn-icon btn-label-info rounded-circle btn-action btn-extend"
                                                data-id="{{ $treasurer->id }}" data-name="{{ $uName }}" data-status="{{ $treasurer->status_jabatan }}"
                                                data-bs-toggle="modal" data-bs-target="#modalExtendTreasurer" data-bs-toggle="tooltip" title="Perpanjang Jabatan">
            ...
            <div class="modal-content border-0 shadow">
                ...
                <form id="formTreasurer" method="POST" action="{{ route('admin.treasurers.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer bg-light px-4 py-3 border-top">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill" id="btnSubmitModal"><i class="ti tabler-device-floppy me-1"></i> Simpan Bendahara</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="modalExtendTreasurer" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content border-0 shadow">
                ...
                <form id="formExtendTreasurer" method="POST">
                    @csrf
                    <div class="modal-body px-4 py-4">
                        <div class="alert alert-info d-flex align-items-center p-3 mb-4">
                            <i class="ti tabler-info-circle icon-lg me-2"></i>
                            <div class="small">
                                Anda akan memperbarui jabatan <strong id="namaExtendTreasurer"></strong>. Riwayat lama akan otomatis diarsipkan.
                            </div>
                        </div>
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label required">Status Jabatan Baru</label>
                                <select class="form-select" id="extendStatusJabatanTreasurer" name="status_jabatan" required>
                                    <option value="tetap">Bendahara Definitif (Tetap)</option>
                                    <option value="plt">Pelaksana Tugas (Plt)</option>
                                    <option value="plh">Pelaksana Harian (Plh)</option>
                                </select>
    ...
    <script type="module">
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.btn-extend').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.getElementById('namaExtendTreasurer').innerText = this.dataset.name;
                    let statusSelect = document.getElementById('extendStatusJabatanTreasurer');
                    if(statusSelect) {
                        statusSelect.value = this.dataset.status;
                    }
                    let urlAction = "{{ route('admin.treasurers.extend', ':id') }}";
                    document.getElementById('formExtendTreasurer').action = urlAction.replace(':id', this.dataset.id);
                });
            });
